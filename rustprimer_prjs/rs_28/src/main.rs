...
fn main() -> Result<(), std::io::Error> {
  // Coding Style
  {
    ...
    //  基本上，这意味着用 “Returns” 而不是 “Return”。例如：
    ...
    //     Lifetimes	                  短的小写: 'a
    //  在 CamelCase中, 首字母缩略词被当成一个单词：用 Uuid 而不是UUID。在 snake_case 中，首字母缩略词全部是小写： is_xid_start。
    ...
    // 
    // Getter/setter 方法
    //  一些数据类型不希望提供对它们的域的直接访问，但是提供了 “getter” 和 “setter” 方法用于操纵域状态（经常提供检查或其他功能）。域 foo: T 的约定是：
